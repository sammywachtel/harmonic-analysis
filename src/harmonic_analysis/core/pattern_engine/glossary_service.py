...
class GlossaryService:
    ...
    def __init__(self, glossary_path: Optional[str] = None):
        ...
        Args:
            glossary_path: Path to glossary JSON file. If None, uses default location.
        """
        ...
        try:
            with open(glossary_path, 'r') as f:
                self.glossary: Dict[str, Any] = json.load(f)
    ...
    def get_cadence_explanation(self, pattern_name: str) -> Optional[Dict[str, Any]]:
        ...
        if cadence_type and cadence_type in self.glossary.get('cadences', {}):
            ...
            if 'soprano on' in pattern_name.lower():
                ...
                if soprano_degree:
                    cadence_info['soprano_context'] = self._get_soprano_context(soprano_degree)
    ...
    def get_term_definition(self, term: str) -> Optional[str]:
        ...
        sections = ['terms', 'functions', 'voices', 'chord_positions',
                   'figured_bass_notation', 'non_cadential_progressions']

        term_lower = term.lower().replace(' ', '_')

        for section in sections:
            if section in self.glossary and term_lower in self.glossary[section]:
                return self.glossary[section][term_lower]

        return None

    def get_scale_degree_info(self, degree: int, key: str = "C major") -> Dict[str, Any]:
    ...
    def explain_pattern_result(self, pattern_match: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'evidence' in pattern_match and isinstance(pattern_match['evidence'], list):
            ...
            if roman_explanations:
                explanation['educational_context']['chord_functions'] = roman_explanations
        ...
        if family:
            ...
            if family_explanation:
                explanation['educational_context']['pattern_family'] = family_explanation

        return explanation

    def get_pattern_teaching_points(self, pattern_matches: List[Dict[str, Any]]) -> List[str]:
        ...
        for match in pattern_matches:
            ...
            if family == 'cadence':
                if 'pac' in pattern_name:
                    teaching_points.append(
                        "Perfect Authentic Cadence (PAC) provides the strongest sense of closure "
                        "because both chords are in root position and the soprano resolves to the tonic."
                    )
                elif 'iac' in pattern_name:
                    teaching_points.append(
                        "Imperfect Authentic Cadence (IAC) is weaker than PAC because either "
                        "a chord is inverted or the soprano doesn't end on the tonic."
                    )
                elif 'half' in pattern_name:
                    teaching_points.append(
                        "Half Cadence creates expectation by ending on the dominant (V), "
                        "leaving the phrase feeling incomplete and needing continuation."
                    )
                elif 'plagal' in pattern_name:
                    teaching_points.append(
                        "Plagal Cadence (IV-I) has a distinctive sound often associated with "
                        "hymns and provides gentler closure than authentic cadences."
                    )
            ...
            # Sequence teaching points
                ...
                if 'circle of fifths' in pattern_name:
                    teaching_points.append(
                        "Circle of fifths progressions create strong forward momentum through "
                        "root motion by descending fifths (or ascending fourths)."
                    )
    ...
    def _get_soprano_context(self, degree: int) -> str:
        ...
        if degree == 1:
            return "Soprano ending on scale degree 1 (tonic) provides the strongest closure."
        elif degree == 3:
            return "Soprano ending on scale degree 3 (mediant) creates weaker closure than tonic."
        elif degree == 5:
            return "Soprano ending on scale degree 5 (dominant) provides less stable closure."
        else:
    ...
    def _get_family_explanation(self, family: str) -> Optional[str]:
        ...
        family_explanations = {
            'cadence': "Cadences are harmonic progressions that provide closure or punctuation in musical phrases.",
            'sequence': "Sequences are patterns where a melodic or harmonic idea is repeated at different pitch levels.",
            'prolongation': "Prolongational patterns extend or elaborate a single harmony over time.",
            'chromatic_pd': "Chromatic predominants use altered chords to enhance motion toward the dominant.",
            'mixture_modal': "Modal mixture borrows chords from the parallel mode (major/minor) for color."
        }
