...
class LowLevelEventExtractor:
    ...
    def extract_events(
        self, tokens: List[Token], chord_symbols: List[str], key_center: str
    ) -> LowLevelEvents:
        ...
        voice_leading = self._infer_voice_leading_events(
    ...
    def _infer_voice_leading_events(
        self, tokens: List[Token], chord_symbols: List[str], tonic_pc: int
    ) -> MelodicEvents:
        ...
        melodic_events.voice_7_to_1 = self._detect_7_to_1_inference(
            tokens, chord_symbols
        )
        melodic_events.fi_to_sol = self._detect_fi_to_sol_inference(
            tokens, chord_symbols
        )
        melodic_events.le_to_sol = self._detect_le_to_sol_inference(
            tokens, chord_symbols
        )
    ...
    def _detect_4_to_3_inference(self, tokens: List[Token]) -> List[bool]:
        ...
        for i in range(len(tokens) - 1):
            ...
            if (
                current_roman.startswith("i")
                and (
                    "64" in current_roman
                    or "6/4" in current_roman
                    or "⁶⁴" in current_roman
                )
                and next_roman.startswith("v")
                and next_token.role == "D"
            ):
    ...
    def _detect_7_to_1_inference(
        self, tokens: List[Token], chord_symbols: List[str]
    ) -> List[bool]:
        ...
        for i in range(len(tokens) - 1):
            ...
            if (
                current_token.role == "D"
                and (current_roman.startswith("v") and "7" in current_chord.lower())
                and next_token.role == "T"
                and next_roman.startswith("i")
            ):
    ...
    def _detect_fi_to_sol_inference(
        self, tokens: List[Token], chord_symbols: List[str]
    ) -> List[bool]:
        ...
        for i in range(len(tokens) - 1):
            # current_token = tokens[i]  # Not currently used
            next_token = tokens[i + 1]
            current_chord = chord_symbols[i] if i < len(chord_symbols) else ""
            ...
            is_aug6 = any(
                indicator in current_chord.lower() for indicator in aug6_indicators
            )
    ...
    def _detect_le_to_sol_inference(
        self, tokens: List[Token], chord_symbols: List[str]
    ) -> List[bool]:
        ...
        for i in range(len(tokens) - 1):
            ...
            if (
                current_token.role == "PD"
                and current_roman.startswith("ii")
                and "6" in current_roman
                and next_token.role == "D"
                and next_roman.startswith("v")
            ):
    ...
    def _parse_note_pc(self, note: str) -> int:
        """Parse a note token like 'E#', 'Gb', 'C##', 'Dbb' (Unicode ♯/♭ supported)
        to a pitch class 0–11.
        Only the leading letter A–G and a run of accidentals are considered;
        any trailing quality/octave is ignored.
        """
        if not note:
            return 0
        s = note.strip()
        # Normalize Unicode accidentals and double symbols
        s = s.replace("♯", "#").replace("♭", "b").replace("𝄪", "##").replace("𝄫", "bb")
        # Take leading letter
    ...
    def _detect_root_motion(self, tokens: List[Token]) -> List[str]:
        ...
        for i in range(1, len(tokens)):
            # prev_motion = tokens[i - 1].bass_motion_from_prev  # Not currently used
    ...
    def _detect_bass_degrees(
        self, bass_pcs: List[int], tonic_pc: int
    ) -> List[Optional[str]]:
    ...
    def _detect_cadential64(
        self, tokens: List[Token], bass_pcs: List[int], tonic_pc: int
    ) -> List[bool]:
        ...
        for i, token in enumerate(tokens):
            ...
            if roman.startswith("i") and (
                "64" in roman or "6/4" in roman or "⁶⁴" in roman
            ):
                ...
                if i < len(bass_pcs):
                    ...
                    if bass_pc == dominant_pc:
                        # For cadential 6/4, the chord typically functions as
                        # dominant preparation
                        # but the role might still be T (tonic) because it's
                        # still I chord
    ...
    def _detect_circle_of_fifths_chains(
        self, events: LowLevelEvents, min_length: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            events: LowLevelEvents containing `root_motion` (one entry per chord;
                first entry may be empty).
            min_length: Minimum number of consecutive qualifying transitions to
                count as a chain.
                        Note this counts *transitions*, so a chain with 3
                        transitions spans 4 chords.
        ...
        Returns:
            A descriptor dict identical to `get_circle_of_fifths_descriptor`, e.g.:
            {
                "type": "circle5",
                "min_length": 3,
                "chains": [{"start": 2, "length": 4}, ...],  # start index and
                # number of chords in chain
                "found": true
            }
        """
        # Keep single source of truth: delegate to the module-level helper so
...
def get_circle_of_fifths_descriptor(
    events: LowLevelEvents, min_length: int = 3
) -> Dict[str, Any]:
    ...
    for i, motion in enumerate(events.root_motion):
        if motion == "-5" or motion == "+4":  # Fifth motion
            ...
            transitions_count = (
                current_chord_count - 1 if current_chord_count > 0 else 0
            )
