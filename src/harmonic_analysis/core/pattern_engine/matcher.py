...
from typing import List, Dict, Any, Optional, Set, Callable, TypedDict, Protocol
...
class MatchResult:
    ...
    @classmethod
    def success_result(cls, score: float, evidence: List[StepEvidence]) -> 'MatchResult':
        """Create successful match result."""
        return cls(success=True, score=score, evidence=evidence)

    @classmethod
    def failure_result(cls, reason: MatchFailureReason, debug_info: Dict = None) -> 'MatchResult':
...
class SequenceMatchStrategy(MatchStrategy):
    ...
    def match(self, pattern: Pattern, tokens: List[Token]) -> MatchResult:
        ...
        return MatchResult.success_result(
            score=final_score,
            evidence=evidence
        )

    def _effective_role(self, tok: Token, pattern: Pattern, next_tok: Optional[Token] = None) -> str:
        """Compute token role possibly remapped by pattern constraints (e.g., treat_I64_as_D).
    ...
    def _step_matches_token(self, step: StepSpec, tok: Token, pattern: Pattern, next_tok: Optional[Token] = None) -> bool:
        """Check if token satisfies step requirements with substitutions."""
        # Descriptor-based steps handled by different strategy
        if step.descriptor:
            return False

        # Check functional role match with flexibility for T_or_D and effective role remapping
        if step.role:
        ...
        if step.roman_root_any_of:
            ...
            # If any allowed value specifies an inversion/figure, require exact root match;
            # otherwise, compare using inversion-insensitive base.
            inversion_sensitive = any(a.endswith(("6","65","64","43","42")) for a in allowed)
            token_cmp_raw = tok.roman_root() if inversion_sensitive else tok.roman_base()
        ...
        if (step.flags_any_of and
            not any(f in tok.flags for f in step.flags_any_of)):
            return False
        ...
        if step.secondary_of:
            if (step.secondary_of != "*" and
                step.secondary_of != tok.secondary_of):
                return False
    ...
    def _calculate_enhanced_score(self, pattern: Pattern, tokens: List[Token],
                                 evidence: List[StepEvidence]) -> float:
        """
        Calculate enhanced score with bonuses and penalties.

        Rewards good voice leading, penalizes awkward progressions.
        """
        base_score = pattern.score.get("base", 1.0)
        ...
        if "smooth_voice_leading" in bonuses:
            smooth_motions = sum(1 for t in tokens[1:]
                               if t.bass_motion_from_prev is not None and
                                  abs(t.bass_motion_from_prev) <= 2)  # Step or skip
            if smooth_motions > 0:
        ...
        if "strong_cadence" in bonuses and len(tokens) >= 2:
            ...
            if (final_token.role == "T" and
                final_token.soprano_degree == 1):  # Soprano on tonic
                final_score += bonuses["strong_cadence"]

        # Awkward leap penalties
        if "large_leap_penalty" in penalties:
            large_leaps = sum(1 for t in tokens[1:]
                            if t.bass_motion_from_prev is not None and
                               abs(t.bass_motion_from_prev) > 5)  # Larger than fourth
...
class DescriptorMatchStrategy(MatchStrategy):
    ...
    def match(self, pattern: Pattern, tokens: List[Token]) -> MatchResult:
        ...
        if self.descriptor_registry.check(descriptor, tokens):
...
class BassMotionConstraint(ConstraintChecker):
    ...
    def check(self, tokens: List[Token], constraint_value: Any) -> bool:
        ...
        return any(t.bass_motion_from_prev in constraint_value
                  for t in tokens if t.bass_motion_from_prev is not None)
...
class SopranoConstraint(ConstraintChecker):
    ...
    is intentionally permissive to support macro patterns that don't fix the cadence tone.
    """

    def check(self, tokens: List[Token], constraint_value: Any) -> bool:
        # constraint_value is list of acceptable soprano scale degrees
        return any((t.soprano_degree in constraint_value)
                  for t in tokens if t.soprano_degree is not None)
...
class PhrygianStepConstraint(ConstraintChecker):
    ...
    def check(self, tokens: List[Token], constraint_value: Any) -> bool:
        if constraint_value:  # Require Phrygian step
            return any(t.bass_motion_from_prev == -1
                      for t in tokens if t.bass_motion_from_prev is not None)
        return True
...
class ArrivalSopranoConstraint(ConstraintChecker):
    ...
    def check(self, tokens: List[Token], constraint_value: Any) -> bool:
        ...
        final_token = tokens[-1]
        return (final_token.soprano_degree is not None and
               final_token.soprano_degree in constraint_value)
...
class DescriptorRegistry:
    ...
    def register(self, name: str, checker: Callable[[List[Token]], bool]):
    ...
    def _circle_of_fifths_checker(self, tokens: List[Token]) -> bool:
        ...
        for token in tokens[1:]:
            ...
            if m is not None and m in (-7, -5):  # accept semitone -7 or diatonic-coded -5
    ...
    def _chromatic_mediant_checker(self, tokens: List[Token]) -> bool:
        ...
        if len(tokens) < 2:
            return False
        chromatic_indicators = ["♭VI", "♭III", "♯iv", "♭ii", "♯IV", "♭VII",
                                 "bVI", "bIII", "#iv", "bii", "#IV", "bVII"]
        for token in tokens:
...
class SubstitutionEngine:
    ...
    def expand_allowed_chords(self, base_chords: List[str],
                             pattern: Pattern) -> Set[str]:
...
class IntervalSchedulingSolver:
    ...
    def solve(self, intervals: List[ScoredInterval]) -> List[ScoredInterval]:
        ...
        for i in range(1, n + 1):
            ...
            # Option 1: Include this interval + best solution up to latest non-overlapping
    ...
    def _find_latest_non_overlapping(self, intervals: List[ScoredInterval],
                                   current_idx: int) -> int:
        """
    ...
    def _backtrack_solution(self, intervals: List[ScoredInterval],
                          choose: List[bool]) -> List[ScoredInterval]:
...
class PatternMatcher:
    ...
    def match(self, tokens: List[Token],
              best_cover: bool = True) -> List[PatternMatch]:
        """
    ...
    def _find_all_matches(self, tokens: List[Token]) -> List[PatternMatch]:
        ...
        for pattern in self.library.patterns:
            # Profile filter: if a pattern declares profiles, it must include the active profile
    ...
    def _cached_pattern_match(self, pattern_id: str, token_signature: str) -> MatchResult:
        """
...
# (get_loaded_pattern_ids, try_match_pattern, etc.) to aid external tooling like
# debug_patterns.py. These are safe to remove once validation is green.
...
class Matcher(PatternMatcher):
    ...
    def get_loaded_pattern_ids(self, limit: Optional[int] = None):
        return super().get_loaded_pattern_ids(limit)
    def get_substitution_engine(self):
        return super().get_substitution_engine()
    def get_current_profile(self) -> str:
        return super().get_current_profile()
    def get_constraint_validator(self):
        return super().get_constraint_validator()
    def get_descriptor_registry(self):
