...
# Canonical key spellings (to avoid enharmonic drift)
CANONICAL_KEY_MAP = {
    # Majors
    "Cb major": "B major",
    "C major": "C major",
...
def canonicalize_key_signature(key: str) -> str:
    """Return a musically canonical key spelling (avoid D# major, Cb major, etc.)."""
    if not key:
        return key
    key = key.strip()
    return CANONICAL_KEY_MAP.get(key, key)
