...
class UnifiedPatternService:
    ...
    async def analyze_with_patterns_async(
        ...
        if inferred_key and chords:
            try:
                ...
                logger.debug(f"🎵 Derived romans with key {inferred_key}: {chords} → {roman_numerals}")
            except Exception as e:
        ...
        if mode_label and envelope.primary:
            ...
            if modal_parent_key != context.key:
                if not hasattr(envelope.primary, 'modal_parent_key'):
                    ...
                    if hasattr(envelope.primary, 'metadata') and envelope.primary.metadata:
                    ...
                    if modal_parent_key != context.key:
                        ...
                        try:
                            ...
                            if modal_envelope.primary:
                                ...
                                logger.debug(f"🎵 Re-analyzed with modal parent {modal_parent_key}: {chords} → {modal_romans}")

                        except Exception as e:
                            logger.warning(f"⚠️ Failed to re-analyze with modal parent key: {e}")
    ...
    def _detect_mode_label(self, roman_numerals: List[str], key: Optional[str], chords: List[str]) -> Optional[str]:
        """
        ...
        roman_signature = ' '.join(roman_numerals)  # Keep original case for contextless patterns
        roman_signature_upper = roman_signature.upper()  # Uppercase version for main signatures

        # Time to tackle the tricky bit: enhanced modal signature detection
        mode_signatures = {
            'mixolydian': ['♭VII', 'BVII'],
            'dorian': ['VI', '♯VI', 'II', 'IV'],  # Natural 6, raised 2 (IV not iv for uppercase)
            'phrygian': ['♭II', 'BII', '♭III'],  # Fixed case consistency
            'lydian': ['♭V', 'BV', '♯IV', '#IV'],
            'aeolian': ['V', '♭VI', '♭VII'],  # Natural minor characteristics (V not v)
            'locrian': ['♭II', '♭V', 'II°', 'V°']  # Diminished characteristics (uppercase)
        }
        ...
        for mode_name, signatures in mode_signatures.items():
            if any(sig in roman_numerals_upper for sig in signatures):
                ...
                if mode_name == 'mixolydian':
                    if key_type == 'major' and ('I' in roman_signature_upper or any(not 'm' in c for c in chords[:2])):
                        return f"{modal_tonic} Mixolydian"
                elif mode_name == 'dorian':
                    if (any(r == 'i' for r in roman_numerals) or any('m' in c and 'maj' not in c for c in chords[:2])):
                        return f"{modal_tonic} Dorian"
                elif mode_name == 'phrygian':
                    if key_type == 'minor' or any('m' in c and 'maj' not in c for c in chords[:2]):
                    ...
                    if '♭II' in roman_signature_upper and ('♭V' in roman_signature_upper or 'V°' in roman_signature_upper):
        ...
        if len(roman_numerals) <= 4 and len(set(roman_numerals)) <= 3:
            if any(r == 'I' for r in roman_numerals_upper) and any(r == '♭VII' for r in roman_numerals_upper):
                return f"{modal_tonic} Mixolydian"
            elif any(r == 'i' for r in roman_numerals) and any(r == '♭II' for r in roman_numerals_upper):
                return f"{modal_tonic} Phrygian"
            elif any(r == 'I' for r in roman_numerals_upper) and any(r == '♭II' for r in roman_numerals_upper):
                return f"{modal_tonic} Locrian"
            elif any(r == 'i' for r in roman_numerals) and any(r == 'IV' for r in roman_numerals_upper):
                return f"{modal_tonic} Dorian"
            elif any(r == 'I' for r in roman_numerals_upper) and any(r == '♯IV' for r in roman_numerals_upper):
                return f"{modal_tonic} Lydian"
    ...
    def _convert_to_modal_parent_key(self, mode_label: str, current_key: str) -> str:
        ...
        try:
            ...
            modal_tonic = parts[0].replace('°', '')  # Remove diminished symbol if present
            ...
            parent_tonic = self._transpose_note(modal_tonic, -interval)  # Subtract semitones
            ...
            logger.warning(f"⚠️ Failed to convert modal parent key for {mode_label}: {e}")
            return current_key

    def _detect_modal_tonic(self, chords: List[str], roman_numerals: List[str], key: str) -> str:
        """
        ...
        first_root = first_chord.split('/')[0].replace('m', '').split('7')[0].split('sus')[0].strip()
        ...
        chord_counts = {}
        for chord in chords:
            root = chord.split('/')[0].replace('m', '').split('7')[0].split('sus')[0].strip()
    ...
    def _normalize_lydian_romans(self, roman_numerals: List[str], key: Optional[str]) -> List[str]:
        """
        ...
        for roman in roman_numerals:
            ...
            if modal_context == 'lydian':
                ...
                if roman == '♭V' or roman == 'bV':
                    ...
                    logger.debug(f"🎼 Lydian normalization: {roman} → {normalized_roman}")
                elif roman.startswith('♭V') and len(roman) > 2 and not roman.startswith('♭VII'):
                    extensions = roman[2:]
                    normalized_roman = f'♯IV{extensions}'
                    normalized.append(normalized_roman)
                    logger.debug(f"🎼 Lydian normalization: {roman} → {normalized_roman}")
                elif roman.startswith('bV') and len(roman) > 2 and not roman.startswith('bVII'):
                    extensions = roman[2:]
                    normalized_roman = f'♯IV{extensions}'
                    normalized.append(normalized_roman)
                    logger.debug(f"🎼 Lydian normalization: {roman} → {normalized_roman}")
    ...
    def _detect_modal_context(self, roman_numerals: List[str], key: Optional[str]) -> Optional[str]:
        """
        ...
        if key_type == 'major':
            ...
            if any(sig in roman_signature for sig in ['♭V', 'BV']) and 'I' in roman_signature:
                return 'lydian'
            # Check for Locrian signatures (from major parent)
            elif any(sig in roman_signature for sig in ['VII', '♭II']) and not any(sig in roman_signature for sig in ['♭VII']):
                return 'locrian'
        else:  # minor key
            # Check for Locrian from minor context
            if any(sig in roman_signature for sig in ['♭II', '♭V']) and 'i' in roman_signature:
                return 'locrian'
    ...
    def _infer_key_from_progression(self, chords: List[str]) -> str:
        ...
        def extract_root_and_quality(chord: str) -> tuple[str, str]:
            ...
            if 'm' in base_chord and 'maj' not in base_chord.lower():
                ...
                root = base_chord.replace('m', '').split('7')[0].split('9')[0].split('11')[0].split('13')[0]
                return root.strip(), 'minor'
            else:
                # Major or unspecified (treat as major)
                root = base_chord.split('7')[0].split('9')[0].split('maj')[0].split('sus')[0].split('add')[0]
        ...
        if first_quality == 'minor' and first_root in common_keys:
            ...
            elif 'lydian' in modal_signatures or any('b' in c.lower() or '#' in c.lower() for c in chords[1:]):
    ...
    def _detect_modal_signatures(self, chords: List[str]) -> List[str]:
        ...
        if any(pair in [('C', 'BB'), ('G', 'F'), ('D', 'C')] for pair in zip(chords, chords[1:]) if len(chords) > 1):
        ...
        if any(pair in [('AM', 'BB'), ('DM', 'EB')] for pair in zip(chords, chords[1:]) if len(chords) > 1):
    ...
    def _detect_functional_major_key(self, chords: List[str]) -> Optional[str]:
        ...
        def get_root(chord: str) -> str:
            return chord.split('/')[0].replace('m', '').split('7')[0].split('9')[0].strip()
        ...
        if len(chords) >= 3:
            ...
            if self._matches_IV_V_vi_pattern(roots):
                ...
                minor_third_up = self._transpose_note(roots[-1], 3)  # 3 semitones = minor 3rd
    ...
    def _transpose_note(self, note: str, semitones: int) -> str:
        ...
        note_to_index = {'C': 0, 'C#': 1, 'DB': 1, 'D': 2, 'D#': 3, 'EB': 3, 'E': 4, 'F': 5,
                        'F#': 6, 'GB': 6, 'G': 7, 'G#': 8, 'AB': 8, 'A': 9, 'A#': 10, 'BB': 10, 'B': 11}
