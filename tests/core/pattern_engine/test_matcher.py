...
def _load_tests():
    with open(DATA_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    aliases = { _norm_text(k): [_norm_text(a) for a in v] for k, v in data.get("aliases", {}).items() }
...
def _found_sets(analysis):
    ...
    fams  = [_norm_text(m.get("family", "")) for m in analysis.get("pattern_matches", [])]
...
def test_patterns_from_json(case):
    ...
    best_cover = case.get("best_cover", False)  # existence-based by default for validation
