...
class TestScaleAnalysisErrorHandling:
    ...
    def test_missing_key_hint_error(self):
        ...
        with pytest.raises(ValueError, match="Scale analysis requires key_hint parameter"):
    ...
    def test_invalid_scale_notes_error(self):
        ...
        with pytest.raises(ValueError, match="Scale analysis failed"):
            self.service.analyze_with_patterns(
                notes=['X', 'Y', 'Z'],  # Invalid note names
                key_hint='C major'
            )
    ...
    def test_scale_key_mismatch_error(self):
        ...
        with pytest.raises(ValueError, match="Scale analysis failed"):
            self.service.analyze_with_patterns(
                notes=['C', 'C#', 'D#', 'E', 'F', 'G', 'A'],  # Invalid mode pattern - not a standard mode
                key_hint='C major'
            )
...
class TestScaleAnalysisEdgeCases:
    ...
    def test_incomplete_scale_analysis(self):
        ...
        with pytest.raises(ValueError, match="Scale analysis failed"):
            self.service.analyze_with_patterns(
                notes=['C', 'D', 'E'],  # Just first three notes - insufficient for mode detection
                key_hint='C major'
            )
    ...
    def test_extended_scale_analysis(self):
        ...
        with pytest.raises(ValueError, match="Scale analysis failed"):
            self.service.analyze_with_patterns(
                notes=['C', 'D', 'E', 'F', 'G', 'A', 'B', 'C'],  # 8 notes - not a standard mode
                key_hint='C major'
            )
    ...
    def test_case_insensitive_note_names(self):
        ...
        result = self.service.analyze_with_patterns(
            notes=['c', 'D', 'e', 'F', 'g', 'A', 'b'],  # Mixed case
            key_hint='C major'
        )
...
class TestScaleAnalysisIntegrationWithPatterns:
    ...
    def test_scale_degree_regression(self):
        ...
        test_cases = [
            ...
            (['G', 'A', 'B', 'C', 'D', 'E', 'F'], 'G mixolydian', [1, 2, 3, 4, 5, 6, 7]),
        ...
        for notes, key_hint, expected_degrees in test_cases:
            ...
            from harmonic_analysis.core.pattern_engine.token_converter import normalize_scale_input

            scale_data = normalize_scale_input(notes, key_hint)

            # Verify the scale was processed correctly
            assert scale_data['is_valid'], f"Scale {notes} with key {key_hint} should be valid"
            assert scale_data['canonical_notes'] == notes, f"Notes should match input"
            ...
            assert len(scale_data['scale_degrees']) == len(notes), f"Should have one degree per note"
            assert all(isinstance(d, int) for d in scale_data['scale_degrees']), "All degrees should be integers"
